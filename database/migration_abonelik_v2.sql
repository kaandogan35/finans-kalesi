...
--   3. Mevcut 'ucretsiz' kullanıcılar → 'deneme' + 7 gün deneme süresi
...
ALTER TABLE `sirketler`
    ADD COLUMN `deneme_bitis` DATETIME DEFAULT NULL COMMENT '7 günlük deneme bitiş tarihi'
...
-- 3. Mevcut 'ucretsiz' kullanıcıları 'deneme'ye çevir + 7 gün süre ver
UPDATE `sirketler`
SET `abonelik_plani` = 'deneme',
    `deneme_bitis` = DATE_ADD(NOW(), INTERVAL 7 DAY)
...
--   deneme_bitis   = NOW() + 7 gün
-- 7 gün sonunda ödeme yapılmazsa kısıtlı erişim uygulanır
-- =====================================================
